...
import util.tools.http
import util.tools.hujson2 as json
import mimetypes
import os
import urlparse
...
def get_list(endpoint, queuename, credentials=None):
    ...
    status, headers, content = util.tools.http.fetch(url, method='GET', headers=headers, credentials=credentials)
    if status != 200:
...
def push(endpoint, queuename, guid, document, content_type='application/octet-stream', credentials=None):
    ...
    status, headers, content = util.tools.http.fetch(url,
                                                  method='POST',
                                                  content=document,
                                                  headers=headers,
...
def pull(url, credentials=None):
    ...
    status, headers, content = util.tools.http.fetch(url, method='GET', credentials=credentials)
    if status != 200:
        raise FMTPError(status, content)
    return content
...
def acknowledge(endpoint, queuename, guid, credentials=None):
    ...
    url = build_url(endpoint, queuename, guid)
    status, headers, content = util.tools.http.fetch(url, method='DELETE', credentials=credentials)
