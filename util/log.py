...
def set_config(LOG_FILE = None,MODE = 'a',PRINT_LEVEL = 'DEBUG',LOG_LEVEL = 'ERROR',
               LOG_FORMAT = '%(asctime)s [%(levelname)s]: %(message)s',BACKUP_COUNT = 5,
               LIMIT = 20480 , WHEN = None):
    set_logger(filename = LOG_FILE,mode = MODE,level = '%s:%s' % (PRINT_LEVEL,LOG_LEVEL),
               fmt = LOG_FORMAT, backup_count = BACKUP_COUNT, limit = LIMIT, when = WHEN)
