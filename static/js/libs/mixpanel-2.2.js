...
// This key is deprecated, but we want to check for it to see whether aliasing is allowed.
    ...
    /** @const */       , RESERVED_PROPERTIES       = [SET_QUEUE_KEY, ADD_QUEUE_KEY, APPEND_QUEUE_KEY, PEOPLE_DISTINCT_ID_KEY, ALIAS_ID_KEY];
    ...
    _.JSONEncode = (function() {
        if (!_.isUndefined(window.JSON) && JSON.stringify) {
    ...
    _.JSONDecode = (function() { // https://github.com/douglascrockford/JSON-js/blob/master/json_parse.js
        if (!_.isUndefined(window.JSON) && JSON.parse) {
            return JSON.parse;
        }
    ...
    /**
     * mixpanel.identify(unique_id:string, _set_callback:function, _add_callback:function)
     *
     ...
     */
    MixpanelLib.prototype.identify = function(unique_id, _set_callback, _add_callback, _append_callback) {
        ...
        this['people']._flush(_set_callback, _add_callback, _append_callback);
    };
    ...
    /**
     * mixpanel.alias(alias:string, original:string)
     *
    ...
    /* mixpanel.people.append(prop, by)
     *
     * append to lists
     *
     * usage:
    ...
    /* mixpanel.people.track_charge(amount:number [, callback:function])
     *
     ...
     *
     */
    MixpanelPeople.prototype.track_charge = function(amount, callback) {
    ...
    MixpanelPeople.prototype._flush = function(_set_callback, _add_callback, _append_callback) {
        var _this = this,
            $set_queue = this._mixpanel.cookie._get_queue(SET_ACTION),
            $add_queue = this._mixpanel.cookie._get_queue(ADD_ACTION),
            $append_queue = this._mixpanel.cookie._get_queue(APPEND_ACTION);

        if (!_.isUndefined($set_queue) && !_.isEmptyObject($set_queue)) {
        ...
        if (!_.isUndefined($add_queue) && !_.isEmptyObject($add_queue)) {
        ...
        if (!_.isUndefined($append_queue) && $append_queue.length) {
            var unprocessed = [];
            function append_next() {
                if ($append_queue.length) {
                    var $append = $append_queue.shift();
                    _this.append($append, function(response, data) {
                        // push the item onto the unprocessed queue if
                        // it fails
                        if (response == 0) { unprocessed.push($append); }
                    ...
                    // we reached the end of the list add all the
                    // unprocessed items back onto the append queue
                    var append_key = _this._mixpanel.cookie._get_queue_key(APPEND_ACTION);
                    _this._mixpanel._register_single(append_key, unprocessed);
                }
            };
